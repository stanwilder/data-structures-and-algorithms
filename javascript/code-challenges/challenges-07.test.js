...
const sortStarWarsCharacters = (starWarsArr) => {
  return starWarsArr.sort(function(a,b){return b.height - a.height;});
...
const howMuchPencil = (str) => {
  ...
  for (let i = 0; i < stringArr.length; i++){
    let shavedPencil = stringArr.slice(i);
    // console.log(shavedPencil);
    shavedPencil = shavedPencil.join('');
    // console.log(shavedPencil);
    result.push(shavedPencil);
  }
  result.push('');
  console.log(result);
  return result;
...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach((ingredient)=> {
    let num = ingredient.slice(ingredient.' ') + 1;
    let num2 = num
  });
  // Solution code here...
  return result;
