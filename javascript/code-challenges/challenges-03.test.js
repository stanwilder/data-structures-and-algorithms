...
const typeNum = (arr) => {
  let newArr = arr.filter(name => typeof(name) === 'number');
...
const containsAnd = (arr) => {
  let newArr = arr.filter(word => word.includes('and'));
  // Solution code here...
  return newArr;
};
...
const oddValues = (arr) => {
  let newArr = arr.filter(odds => odds % 2 !== 0);
  // Solution code here...
  return newArr;
};
...
const notInFirstArray = (forbiddenValues, arr) => {
  let newArr = arr.filter(filtered => !forbiddenValues.includes(filtered));
