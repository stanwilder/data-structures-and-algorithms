...
const raisedToTheThird = (arr) => {
  let num = [];
  arr.forEach(thirdPow => {
    let newNum = Math.pow(thirdPow, 3);
    num.push(newNum);
...
const addOne = (arr) => {
  // arr.forEach(plusOne => {}); Blueprint
  let newNum = arr.map(plusOne => plusOne + 1);
...
const addQuestion = (arr) => {
  let question = arr.map(questionMark => questionMark + '?');
  return question;
